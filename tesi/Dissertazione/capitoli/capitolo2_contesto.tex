\chapter{Dall'interrogazione formale alla ricerca semantica}\label{chap:contesto}

Il capitolo esplora il background scientifico e tecnologico alla base del lavoro proposto.
Partiamo dal \textit{problema di fondo}: come fornire accesso content-aware a repository RDF di dimensioni massive per utenti privi di competenze tecniche formali?
...
\section{Lo stato dell'arte}\label{sec:statoarte}
\subsection{Il contesto: Digital Humanities e la barriera della ricerca}\label{subsec:dh_barrier}

Nel dominio delle Digital Humanities (DH) e della gestione del patrimonio culturale, la ricerca all'interno di repository RDF è un ostacolo per chi manca di competenze tecniche avanzate.
...
In questo scenario, la ricerca esplorativa favorisce la \textit{serendipity}, la scoperta inattesa di pattern rilevanti durante la navigazione dei dati, che gli strumenti tradizionali ostacolano.
...
Analizziamo i principi che giustificano il ricorso a un'interfaccia conversazionale mediata da LLM.\@
    ...
    Questo si lega al concetto di ``findability'' (trovabilità)\cite{Ros07}. Lo scopo del sistema è rendere le risorse trovabili, non solo accessibili, nel momento del bisogno.
...
\section{Retrieval-Augmented Generation (RAG) come soluzione tecnologica}\label{sec:rag}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Augmentation}: lo schema e gli esempi di predicati vengono forniti al modello Quenya come parte del prompt di sistema;
        \item \textbf{Generation}: il modello Quenya produce una query SPARQL coerente con lo schema;
...
In questo capitolo abbiamo analizzato il contesto scientifico e tecnologico alla base del lavoro:
    ...
    Il \cref{chap:prototipo} presenta nel concreto il Quenya Query Builder: il dominio di applicazione (DH.ARC e il Datavault), il profilo degli utenti target, l'architettura funzionale del sistema, e i casi d'uso che ne dimostrano il valore nel contesto umanistico.