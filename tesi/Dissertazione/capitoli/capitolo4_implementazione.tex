...
Questo capitolo illustra l'architettura tecnica e le scelte implementative del motore di Quenya. Mentre il \cref{chap:prototipo} descriveva il sistema dal punto di vista dell'utente finale, qui analizziamo i componenti software, lo stack tecnologico e le metodologie di addestramento alla base dell'applicativo.
...
\section{Stack tecnologico: Fedora e Blazegraph}\label{sec:stack}
  \begin{figure}[htbp]
    ...
    \resizebox{\textwidth}{!}{
      ...
      \node[box, right=of backend] (llm) {\textbf{Modello Quenya} \\ (Phi-3-mini)};
  ...
  L'infrastruttura dati su cui si appoggia Quenya è costituita da Fedora
   e Blazegraph \citeweb{Bla24}. 
  Blazegraph opera come triplestore per il Datavault e fornisce l'endpoint SPARQL interrogato dal sistema. La modellazione architetturale del Datavault e le relative pipeline semantiche (Semantic ETL) per l'estrazione e l'allineamento dei metadati sono state ampiamente documentate e realizzate nel lavoro precedente di Farokhi Darani\cite{Far25}.
  Il nostro lavoro è consistito nel creare un layer intermedio per dialogare con Blazegraph, astraendo la sintassi SPARQL dall'interfaccia utente.
\newpage
  \subsection{I tre layer del sistema}\label{subsec:tre_layer}
    ...
    \begin{itemize}
      \item \textbf{Presentazione (Datavault Browser)}: applicazione React con due barre di ricerca testuale, una per una ricerca google-like \code{<SearchField/>} e una per una ricerca avanzata con il tool \code{<QuenyaSearchBar/>} selezionabili da un bottone centrato sopra di esse.
      ...
      \item \textbf{Applicativo (Quenya Query Builder backend, FastAPI)}: il componente centrale che riceve la domanda naturale, dirige il flusso:
      ...
      \item \textbf{Data Layer (Datavault / Blazegraph)}: immagazzina i milioni di triple RDF del Datavault, esegue la query SPARQL ricevuta dal backend e ne restituisce i risultati grezzi.
    \end{itemize}
  ...
  \subsection{Il flusso end-to-end di una richiesta}\label{subsec:flusso_e2e}
    ...
    \begin{enumerate}
      ...
      \item Il backend costruisce il prompt completo (system prompt con le regole di generazione + domanda dell'utente, si veda \cref{subsec:regole}) e lo sottopone a Quenya.\@
  ...
  \subsection{Scelte tecnologiche principali}\label{subsec:scelte_tech}
    ...
    \begin{itemize}
      \item \textbf{React}\citeweb{Rea13} (frontend): per la parte di presentazione, già descritta funzionalmente nel \cref{chap:prototipo}.
      \item \textbf{FastAPI}\citeweb{Ram18} (backend): scelto per la type safety offerta da Pydantic, la documentazione automatica, le performance asincrone e la facilità di integrazione diretta con PyTorch/Transformers per servire il modello.
      \item \textbf{Phi-3-mini}\citeweb{Mic24} (modello base): dimensione contenuta (3.8 miliardi di parametri) che rende fattibile il fine-tuning mantenendo buone performance di base su compiti di generazione strutturata.
      \item \textbf{Unsloth + LoRA} (fine-tuning): Unsloth\citeweb{Uns24} ottimizza il fine-tuning e riduce l'impronta in memoria; LoRA (Low-Rank Adaptation) permette di adattare il modello aggiornando solo un sottoinsieme di parametri, abbattendo i costi computazionali.
      \item \textbf{Blazegraph}\citeweb{Bla24} (triplestore): tecnologia già in uso presso DH.ARC per il Datavault, con supporto nativo a SPARQL 1.1\citeweb{W3c13} e scalabilità fino a centinaia di milioni di triple.
    \end{itemize}
...
\section{Pipeline di fine-tuning dell'LLM}\label{sec:finetuning}
  L'utilizzo di un LLM generico si è rivelato insufficiente per ottenere traduzioni affidabili dal linguaggio naturale a SPARQL.\@ Un modello addestrato su testo web conosce la sintassi SPARQL, ma non possiede la mappa mentale dello specifico schema del Datavault, rischiando di allucinare predicati inesistenti o ignorare le particolarità dell'ontologia. Abbiamo perciò introdotto una fase di \textit{domain adaptation} tramite fine-tuning.

  \subsection{Dataset generativo e schema SPARQL}\label{subsec:dataset_gen}
    Il dataset di addestramento è stato prodotto in modo sintetico tramite lo script \code{genera\_dataset.py}, usando un approccio a \textbf{template parametrici}. Per ciascuna categoria di query è stata definita una funzione Python che, ricevuti parametri come l'anno o il formato file, restituisce la query SPARQL popolata. 
    La corrispondente domanda in linguaggio naturale viene generata variando lessico e struttura sintattica, esponendo così il modello a una pluralità di formulazioni per il medesimo intento.
    \newpage
...
\end{table}
\vspace{-0.8em}
Durante l'analisi della distribuzione dei predicati nel triplestore di produzione è emerso che il namespace EXIF effettivamente presente in Blazegraph è malformato: \url{http://wwww3org/...}, anziché lo standard \url{http://www.w3.org/...} (mancano i due punti e il prefisso \code{www.} corretto). Il bug è stato confermato eseguendo una query SPARQL di verifica diretta sull'endpoint:
\vspace{0.25cm}
...
\end{lstlisting}
% \newpage
\paragraph{Impatto del bug.} Questo mismatch invalida a cascata le interrogazioni: qualunque query SPARQL basata sul namespace EXIF \textit{canonico} restituisce zero risultati per i predicati EXIF, a prescindere dalla sua validità logica. Poiché tali predicati coprono gran parte delle risorse del Datavault, il difetto comprometteva la maggior parte dei casi d'uso del sistema.
...
\subsection{Configurazione dell'addestramento con Unsloth}\label{subsec:unsloth}
    ...
    \subsubsection{L'algoritmo LoRA}
        ...
        Nel nostro setup abbiamo impostato $r = 16 = \alpha$ e applicato l'aggiornamento sia ai proiettori di attention (\code{q/k/v/o\_proj}) sia a quelli feed-forward (\code{gate/up/down\_proj}). Rispetto all'applicazione standard (limitata all'attention), questa copertura più ampia facilita l'apprendimento del vocabolario di dominio, pur mantenendo contenuto il numero di parametri addestrabili.
...
Il coordinamento delle richieste è affidato a un backend in Python basato su FastAPI.\@
L'engine inietta in modo invisibile il \textit{system prompt} in ogni chiamata, costringendo il modello al ruolo di traduttore tecnico. Abbiamo dedicato particolare cura alla gestione del token EOS (\textit{End Of Sentence}), essenziale per troncare l'output e prevenire accodamenti di testo generato dopo la query SPARQL.\@
...
async def load_model():
  ...
  print("Inizializzazione del motore Quenya...")
  ...
  print("Fusione completata! Modello Quenya pronto...")
\end{lstlisting}
...
\begin{itemize}
  \item \textbf{Doppio token di stop}: Phi-3 impiega il token \code{<|end|>} per chiudere il turno di conversazione, oltre all'EOS standard. Omettere \code{<|end|>} nei \code{terminators} causava cicli di generazione inarrestabili.
  \item \textbf{Generazione deterministica}: \code{do\_sample=False} disattiva il campionamento stocastico. Nel contesto della traduzione verso SPARQL, dove si insegue l'esattezza sintattica piuttosto che la varietà lessicale, l'output deterministico è strettamente necessario.
  \item \textbf{Limite ai token generati}: \code{max\_new\_tokens=250} fornisce sufficiente capienza per query vincolate al template, fungendo anche da rete di salvataggio in caso di fallimento dei token di stop.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Short-circuit evaluation (Out-of-Domain):} \\ 
    Se il testo contiene \code{OUT\_OF\_DOMAIN}, la funzione si arresta e restituisce il JSON di errore. Ciò impedisce che il backend tenti di parsare l'errore come query SPARQL.\@
    
    \item \textbf{Troncamento (\code{re.search}):} \\
    Anche se addestrato a rispondere solo con la \code{SELECT}, occasionalmente il modello anticipava la query con blocchi \code{PREFIX}. La regex estrae solo dal pattern \code{SELECT DISTINCT} in poi, demandando al backend l'iniezione dei namespace corretti.
    
    \item \textbf{Normalizzazione semantica (\code{re.sub}):} \\
    In rari casi il modello ometteva la conversione esplicita \code{STR(?date)} richiesta dalla Regola 6. Avendo Blazegraph bisogno di confronti testuali puri, l'omissione produceva query valide ma senza risultati. La regex rileva la dimenticanza e ripristina il cast \code{STR()} dove necessario. % chktex 36
\end{itemize}
