\chapter{Quenya}\label{chap:prototipo}

    Dopo aver esaminato il contesto nel \cref{chap:contesto}, presentiamo ora il prototipo di Quenya (\textbf{Qu}ery \textbf{E}ngine for \textbf{N}avigating \textbf{Y}our \textbf{A}rchives), un query builder sviluppato per sopperire alle problematiche riscontrate.
...
\section{Il dominio e gli utenti target}\label{sec:dominio}
    ...
    \subsection{Il contesto operativo: DH.ARC e il Datavault}\label{subsec:dharc}
        ...
        All'interno di questo contesto, una delle infrastrutture tecnologiche di conservazione è il \textit{Datavault}, un repository digitale basato sull'architettura Fedora, progettato per supportare l'archiviazione a lungo termine, la conservazione e la gestione strutturata delle risorse digitalizzate. 
        ...
        Questa vastità e omogeneità sintattica rende la ricerca e il recupero di specifiche risorse un'operazione complessa.
    ...
    \subsection{Profilo dell'utente finale: i ricercatori del centro}\label{subsec:utenti}
        ...
        \subsubsection{Caratterizzazione dei bisogni informativi}
            In questo ecosistema, gli utenti non ricercano identificativi esatti o risposte chiuse; adottano comportamenti di ricerca esplorativa, collegando concetti e scoprendo associazioni non immediate tra i documenti.
...
\section{Architettura Funzionale e Interfaccia Utente}\label{sec:architettura}
    ...
    \subsection{L'interfaccia: come si usa il Data Vault Explorer}\label{subsec:interfaccia}
        ...
        Invece di dover navigare attraverso complessi moduli a filtri multipli o di dover studiare a priori l'ontologia sottostante per formulare interrogazioni dirette, l'utente ha ora a disposizione un singolo e intuitivo punto di ingresso: la \textit{Quenya Search Bar}. L'interfaccia è simile ai motori di ricerca, ma il backend interpreta le sfumature del linguaggio naturale traducendo l'intento del ricercatore in una query SPARQL eseguibile.

        \begin{figure}[H]
            \centering
            \includegraphics[width=\textwidth]{img/schermata_principale.png}
            \caption{L'interfaccia principale del Data Vault Explorer con la Quenya Search Bar.}\label{fig:schermata_principale}
        \end{figure}
    ...
    \subsection{Il flusso dell'interazione}\label{subsec:flusso}
        ...
        %     \begin{minipage}[c]{0.17\textwidth}
            ...
            \begin{figure}[htbp]
                ...
                \begin{tikzpicture}[
                    node distance=1.3cm,
                    box/.style={draw=black!70, rectangle, rounded corners, minimum width=5cm, minimum height=1.1cm, align=center, fill=black!5, font=\small},
                ...
                \draw[arrow, dashed] (output.west) -- ++(-1.3,0) |- (input.west) node[pos=0.25, left, font=\footnotesize\itshape, align=right] {Esplorazione\\iterativa};

                \end{tikzpicture}
       
            \caption{Schema del flusso di interazione tra utente, Quenya Query Builder e Datavault.}\label{fig:flusso_interazione}
        \end{figure}
\newpage
        ...
        \begin{enumerate}
            ...
            \item \textbf{Elaborazione}: il testo naturale viene inviato al modello Quenya fine-tuned che, essendo a conoscenza dello schema RDF e dell'ontologia del Datavault, produce autonomamente la query SPARQL corrispondente.
            \item \textbf{Esecuzione e Output}: la query SPARQL viene inoltrata a Blazegraph. L'utente non è esposto al codice formale (salvo esplicita richiesta), ma visualizza esclusivamente i risultati pertinenti nell'interfaccia grafica.
        \end{enumerate}
    ...
    \subsection{I componenti del sistema e le loro responsabilità}\label{subsec:componenti}
        ...
        \begin{enumerate}
            ...
            \item \textbf{Backend (Quenya Query Builder + RAG Engine)}: il componente centrale del sistema. Riceve la domanda, recupera lo schema RDF, genera la query SPARQL tramite il modello e gestisce gli errori semantici (evitando le allucinazioni).
...
\section{Casi d'uso principali ed esempi}\label{sec:casiduso}
  ...
  LIMIT 20\end{lstlisting}
        ...
        \textbf{Risoluzione del problema}: Oltre a delegare all'LLM l'onere della traduzione del linguaggio naturale nel predicato corretto (\code{exif:make}), questo scenario dimostra la robustezza dell'architettura. 
