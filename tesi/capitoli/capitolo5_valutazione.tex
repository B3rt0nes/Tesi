...
Per ragioni di disponibilità hardware e per isolare i tempi di inferenza, i parametri di esecuzione del test sono stati strutturati come segue:
...
Questo sbilanciamento va tenuto presente nell'interpretazione dei risultati: le metriche aggregate risultano molto più informative se lette trasversalmente per categoria (\cref{subsec:metriche_categoria}) piuttosto che come media generale schiacciata dalla classe maggioritaria.
...
La distribuzione è concentrata tra i 7 e i 9 secondi per la maggioranza delle query, con un'evidente coda di valori bassi (minimo 0.65s) verosimilmente corrispondenti a generazioni più brevi; come le risposte \textit{Out-of-Domain}, strutturalmente più corte di una query SPARQL completa.
...
È rilevante osservare che la curva di \textit{validation loss} segue da vicino quella di training senza una divergenza significativa (assenza di un evidente fenomeno di overfitting nella finestra di step osservata), e che entrambe le curve si stabilizzano ben prima del termine del training (intorno allo step 400 su un totale di 1188), suggerendo che il modello avrebbe potuto convergere anche con un numero inferiore di step.
...
Applicando la metodologia descritta in \cref{subsec:nota_metodologica} (confronto sull'insieme completo dei risultati, al netto di \code{ORDER BY}/\code{LIMIT}), le metriche di precision, recall e F1 per categoria sono le seguenti:
...
Anche il riconoscimento delle richieste Out-of-Domain (\cref{subsec:errori}) raggiunge il 100\% di accuratezza: tutti i 57 esempi fuori dominio nel test set sono stati correttamente classificati come tali dal modello. Allo stesso modo, il \code{limit\_match} (corrispondenza esatta del valore richiesto nella clausola \code{LIMIT}) è verificato nel 100\% dei 443 casi non Out-of-Domain in cui almeno una delle due query (generata o di riferimento) include un \code{LIMIT}.
...
A seguito di una verifica sul codice di inferenza (\code{generate\_predictions.ipynb}), è stato confermato che \textbf{il test set costituisce una reale partizione \textit{held-out}}. L'estrazione è avvenuta in modo deterministico applicando un rapporto 95\%/5\% e utilizzando il medesimo seed (\code{3407}) impiegato per la configurazione del training. Questa garanzia metodologica esclude la presenza di \textit{data leakage} o sovrapposizioni: il modello ha elaborato istanze effettivamente mai viste in fase di addestramento. Ciò avvalora l'ipotesi della reale generalizzazione strutturale (punto 1), pur confermando che tale generalizzazione avviene strettamente \textit{in-distribution} rispetto ai template.

Questa osservazione non invalida il risultato---un'accuratezza sintattica ed esecutiva del 100\% sui formati di query coperti dal generatore è comunque un traguardo solido e utile per l'applicativo---ma ne delimita correttamente la portata: la valutazione quantitativa qui presentata misura primariamente la capacità del modello di rispettare in modo affidabile e ripetibile la grammatica e le regole imposte dal system prompt, più che la sua capacità di decodificare formulazioni libere e impreviste di un ricercatore umano. Una valutazione complementare con domande poste in modo spontaneo da utenti reali, non vincolate dai template del generatore, costituisce un'estensione naturale di questo lavoro (\cref{chap:conclusioni}).
...
I risultati presentati in questo capitolo possono essere riassunti in tre punti principali:
