...
\section{Stack tecnologico: Fedora e Blazegraph}\label{sec:stack}
  ...
  L'infrastruttura dati su cui si appoggia Quenya è costituita da Fedora e Blazegraph. 
  Blazegraph opera come triplestore per il Datavault e fornisce l'endpoint SPARQL interrogato dal sistema.
  ...
  \subsection{Scelte tecnologiche principali}\label{subsec:scelte_tech}
    ...
    \begin{itemize}
      ...
      \item \textbf{Phi-3-mini} (modello base): dimensione contenuta (3.8 miliardi di parametri) che rende fattibile il fine-tuning mantenendo buone performance di base su compiti di generazione strutturata.
      \item \textbf{Unsloth + LoRA} (fine-tuning): Unsloth ottimizza il fine-tuning e riduce l'impronta in memoria; LoRA (Low-Rank Adaptation) permette di adattare il modello aggiornando solo un sottoinsieme di parametri, abbattendo i costi computazionali.
...
\begin{description}
    \item[Short-circuit evaluation (Out-of-Domain):] Se il testo contiene \code{OUT\_OF\_DOMAIN}, la funzione si arresta e restituisce il JSON di errore. Ciò impedisce che il backend tenti di parsare l'errore come query SPARQL.\@
    
    \item[Troncamento (\code{re.search}):] Anche se addestrato a rispondere solo con la \code{SELECT}, occasionalmente il modello anticipava la query con blocchi \code{PREFIX}. La regex estrae solo dal pattern \code{SELECT DISTINCT} in poi, demandando al backend l'iniezione dei namespace corretti.
    
    \item[Normalizzazione semantica (\code{re.sub}):] In rari casi il modello ometteva la conversione esplicita \code{STR(?date)} richiesta dalla Regola 6. Avendo Blazegraph bisogno di confronti testuali puri, l'omissione produceva query valide ma senza risultati. La regex rileva la dimenticanza e ripristina il cast \code{STR()} dove necessario. % chktex 36
\end{description}
