...
%
% TODO: tabella dei predicati reali con frequenza (dati dalla query di
% frequenza metadati lanciata su Blazegraph)
%
% \begin{table}[h]
% \centering
% \begin{tabular}{lll}
...
% \midrule
% rdf:type & 2.707.278 & Tipizzazione della risorsa \\
% ldp:contains & 676.186 & Relazione di contenimento container -> risorsa \\
% ebucore:filename & 674.985 & Nome del file \\
% ebucore:hasMimeType & 674.985 & Tipo MIME della risorsa \\
% premis:hasSize & 674.985 & Dimensione del file \\
% exif:dateTime & 674.959 & Data/ora di acquisizione \\
% exif:make / exif:model & 674.959 & Dispositivo di acquisizione \\
% exif:imageWidth / imageLength & 672.607 & Dimensioni in pixel \\
% dc:title & 1.196 & Titolo (solo sui container) \\
% \bottomrule
% \end{tabular}
% \caption{Predicati principali dello schema RDF del Datavault, con frequenza osservata}
% \label{tab:schema_predicati}
% \end{table}
%

Durante l'analisi della distribuzione dei predicati nel triplestore di produzione è emerso che il namespace EXIF effettivamente presente in Blazegraph è malformato: \code{http://wwww3org/2003/12/exif/ns\#}, anziché lo standard \code{http://www.w3.org/2003/12/exif/ns\#} (mancano i due punti e il prefisso \code{www.} corretto). Il bug è stato confermato eseguendo una query SPARQL di verifica diretta sull'endpoint:

\begin{lstlisting}[language=bash, caption=Query di verifica del namespace EXIF reale]
...
\begin{lstlisting}[language=bash, caption=System prompt utilizzato in training e inference (identico in entrambi i file)]
