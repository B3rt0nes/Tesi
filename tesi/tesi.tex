...
\section{Fedora}
Fedora è un progetto open source di DuraSpace. FEDORA sta per Flexible Extensible
Digital Object Repository Architecture ed è un software per la gestione di repository
digitali. FEDORA è progettato per supportare la creazione, la gestione e la conservazione 
a lungo termine di oggetti digitali.
Supporta relazioni semantiche complesse tra oggetti dentro e fuori le repository utilizzando RDF.

Fedora è middleware, il che significa che fornisce un'interfaccia per accedere e gestire 
i dati, ma non si occupa direttamente della memorizzazione dei dati stessi. 
Invece, si integra con vari sistemi di archiviazione e database per garantire la scalabilità 
e la flessibilità nella gestione dei repository digitali.

Si possono costruire framework personalizzati sopra Fedora, oppure ci si può unire ad una
comunità broader come samvera, che è un framework per la gestione di repository digitali 
che utilizza Fedora come sistema di gestione dei dati sottostante. 
Samvera fornisce una serie di strumenti e componenti predefiniti per facilitare la creazione 
di repository digitali, inclusi modelli di dati, interfacce utente e funzionalità di ricerca.

\subsection{RDF}
RDF (Resource Description Framework) è un modello di dati standard per rappresentare 
informazioni su risorse in modo strutturato e semantico. RDF utilizza una struttura a tripla, 
composta da \textbf{soggetto}, \textbf{predicato} e \textbf{oggetto}, per descrivere le 
relazioni tra le risorse. 

Un veloce esempio di una tripla RDF potrebbe essere: 
"Il libro 'La Divina Commedia' è scritto da Dante Alighieri", 
dove \textit{"Il libro 'La Divina Commedia'"} è il soggetto, "è scritto da" è il predicato e "Dante Alighieri" è l'oggetto. RDF è ampiamente utilizzato per rappresentare dati su Internet e per facilitare l'integrazione e la condivisione di informazioni tra diverse fonti.
